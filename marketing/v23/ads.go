...
func (as *AdService) Get(ctx context.Context, id string) (*Ad, error) {
	...
	err := as.c.GetJSON(ctx, fb.NewRoute(Version, "/%s", id).Fields("id", "creative", "name", "account_id", "adset_id", "created_time", "status",
		"adset{id,campaign_id,attribution_spec,daily_budget,name,start_time,end_time,created_time,status,bid_strategy,bid_amount,targeting{age_min,age_max,publisher_platforms,geo_locations,genders,custom_audiences,excluded_custom_audiences,flexible_spec,exclusions}}",
		"adcreatives{id,title,object_story_spec,instagram_user_id,effective_object_story_id}").Limit(1000).String(), res)
	if err != nil {
